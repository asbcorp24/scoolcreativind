...
@forelse($competitions as $c)<tr><td><strong>{{ $c->title }}</strong></td><td>{{ optional($c->starts_on)->format('d.m.Y') }} @if($c->ends_on) — {{ $c->ends_on->format('d.m.Y') }} @endif</td><td>{{ $c->organizer ?: '—' }}</td><td class="text-end"><form method="post" action="{{ route('admin.competitions.delete',$c) }}">@csrf @method('DELETE')<button class="btn btn-sm btn-outline-danger">Удалить</button></form></td></tr>@empty<tr><td colspan="4">Пока пусто.</td></tr>@endforelse
</tbody></table></div></div>
