...
<div class="col-md-3"><label class="form-label">Студия</label><select class="form-select" name="studio_id"><option value="">Общее</option>@foreach($studios as $s)<option value="{{ $s->id }}">{{ $s->title }}</option>@endforeach</select></div><div class="col-md-2"><label class="form-label">Группа</label><select class="form-select" name="study_group_id"><option value="">Для всех</option>@foreach($groups as $g)<option value="{{ $g->id }}">{{ $g->study_year }} год · {{ $g->name }}</option>@endforeach</select></div>
...
<div class="glass-card p-4"><div class="table-responsive"><table class="table admin-table align-middle"><thead><tr><th>Дата</th><th>Время</th><th>Занятие</th><th>Студия</th><th>Группа</th><th></th></tr></thead><tbody>@forelse($lessons as $l)<tr><td>{{ $l->lesson_date->format('d.m.Y') }}</td><td>{{ substr($l->starts_at,0,5) }}–{{ substr($l->ends_at,0,5) }}</td><td><strong>{{ $l->title }}</strong><div class="small text-white-50">{{ $l->teacher_name }}</div></td><td>{{ $l->studio->title ?? 'Общее' }}</td><td>{{ $l->group->name ?? 'Все' }}</td><td class="text-end"><form method="post" action="{{ route('admin.schedule.delete',$l) }}">@csrf @method('DELETE')<button class="btn btn-sm btn-outline-danger">Удалить</button></form></td></tr>@empty<tr><td colspan="6">Пока пусто.</td></tr>@endforelse</tbody></table></div></div>
</div></section>
@endsection
