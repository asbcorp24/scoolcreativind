...
@forelse($studio->media()->orderBy('sort_order')->get() as $m)<tr><td><span class="badge-soft">{{ (['photo'=>'Фото','panorama'=>'360°','video'=>'Видео','model'=>'3D','audio'=>'Аудио'][$m->type] ?? $m->type) }}</span></td><td>{{ $m->title ?: 'Без названия' }}</td><td class="text-truncate" style="max-width:360px">{{ $m->url }}</td><td>{{ $m->is_featured?'Да':'—' }}</td><td class="text-end"><form method="post" action="{{ route('admin.media.delete',$m) }}" onsubmit="return confirm('Удалить материал?')">@csrf @method('DELETE')<button class="btn btn-sm btn-outline-danger">Удалить</button></form></td></tr>@empty<tr><td colspan="5" class="text-white-50">Пока пусто.</td></tr>@endforelse
